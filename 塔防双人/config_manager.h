...
public:
	...
	{
		double speed = 3;
		double normal_attrack_interval = 0.5;
...
public:
	...
	int level_archer = 0;//弓箭手
	int level_axeman = 0;//斧头兵
	int level_gunner = 0;//炮手

	bool is_game_win = false;
	bool is_game_over = false;
	//游戏结束时房子生命值没有归零，那就是胜利
	SDL_Rect rect_tile_map = { 0 };//生成纹理时放置的矩形
	...
	const double num_coin_per_prop = 10;//每捡到一个道具增加多少金币
...
public:
	bool load_level_config(const std::string& path)
	{
		...
		str_stream << file.rdbuf(); file.close();//打开文件，读文件，关闭文件一气呵成
		...
		{
			cJSON_Delete(json_root);//如果这玩意的类型不是一个数组的话，那就delete掉，防止内存泄露
			return false;
		}
		//解析成功之后

		cJSON* json_wave = nullptr;
		cJSON_ArrayForEach(json_wave, json_root)//遍历后者中的前者
		{
			...
			if (json_wave_spawn_list && json_wave_spawn_list->type == cJSON_Array)
			{
				...
				cJSON_ArrayForEach(json_spawn_event, json_wave_spawn_list)
				{
					...
					if (json_spawn_event_enemy_type && json_spawn_event_enemy_type->type == cJSON_String)
					{
						...
						if (str_enemy_type == "Slim") spawn_event.enemy_type = EnemyType::Slim;
						else if (str_enemy_type == "KingSlim") spawn_event.enemy_type = EnemyType::KingSlim;
						else if (str_enemy_type == "Goblin") spawn_event.enemy_type = EnemyType::Goblin;
						else if (str_enemy_type == "GoblinPriest") spawn_event.enemy_type = EnemyType::GoblinPriest;
						else if (str_enemy_type == "Skeleton") spawn_event.enemy_type = EnemyType::Skeleton;	
					}
				}
	...
	bool load_game_config(const std::string& path)
	{
		...
		if (!json_basic || !json_enemy || !json_player || !json_tower
...
private:
	...
	void parse_player_template(PlayerTemplate& tpl, cJSON* json_root)
	{
		...
		if (json_normal_attack_interval && json_normal_attack_interval->type == cJSON_Number)
			tpl.normal_attrack_interval = json_normal_attack_interval->valuedouble;
